...
class DicomViewer(QMainWindow):
    def __init__(self):
        ...
        self._image_obj = None  # Store the matplotlib image object
    ...
    def create_top_bar(self, center_layout):
        ...
        self.backend_combo.setStyleSheet("padding: 6px; border-radius: 4px; font-size: 14px; background-color: #444; color: white; border: 1px solid #555;")
    ...
    def invalidate_cache(self):
        """Invalidate the image cache to force regeneration"""
        self._cached_image_data = None
        self._cached_image_params = (None, None, None, None)
    ...
    def handle_zoom_slider(self, value):
        ...
        self.apply_zoom_to_view()
        
    def apply_zoom_to_view(self):
        if self.current_image_data is not None:
            h, w = self.current_image_data.shape
            center_x = w / 2
            center_y = h / 2
            
            # Calculate new extents based on zoom
            width_half = (w / 2) / self.zoom_factor
            height_half = (h / 2) / self.zoom_factor
            
            new_xlim = (center_x - width_half, center_x + width_half)
            new_ylim = (center_y + height_half, center_y - height_half)
    ...
    def update_display(self):
        ...
        self._image_obj = self.canvas.ax.imshow(
            image_data, 
            cmap='gray', 
            origin='upper', 
            extent=(0, w, h, 0),
...
def main():
    ...
    palette.setColor(QPalette.Base, QColor(68, 68, 68))
