...
# 📦 Create NoctisPro Deployment Package
# This script creates a deployment package to transfer to your Ubuntu 24.04 server

set -e

echo "📦 Creating NoctisPro Deployment Package..."
echo "==========================================="

# Create deployment directory
DEPLOY_DIR="noctis_pro_deployment"
rm -rf $DEPLOY_DIR
mkdir -p $DEPLOY_DIR
...
cp deploy-ubuntu-24.sh $DEPLOY_DIR/
cp start_server.sh $DEPLOY_DIR/ 2>/dev/null || true
...
chmod +x $DEPLOY_DIR/deploy-ubuntu-24.sh
chmod +x $DEPLOY_DIR/start_server.sh 2>/dev/null || true

# Create archive
echo "🗜️ Creating archive..."
tar -czf noctis_pro_deployment.tar.gz $DEPLOY_DIR
...
echo "   Archive:    noctis_pro_deployment.tar.gz"
echo ""
echo "🚀 Next steps:"
echo "   1. Transfer the package to your Ubuntu 24.04 server:"
echo "      scp noctis_pro_deployment.tar.gz user@your-server:~/"
echo ""
echo "   2. On your server, extract and deploy:"
echo "      tar -xzf noctis_pro_deployment.tar.gz"
echo "      cd noctis_pro_deployment"
echo "      sudo bash deploy-ubuntu-24.sh"
echo ""
...
# Show package size
PACKAGE_SIZE=$(du -h noctis_pro_deployment.tar.gz | cut -f1)
