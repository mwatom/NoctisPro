...
class DicomCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None):
        self.fig = Figure(figsize=(8, 8), facecolor='black', dpi=100)
...
class DicomViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Improved Python DICOM Viewer")
        ...
        self._image_object = None  # Store matplotlib image object
    ...
    def create_top_bar(self, center_layout):
        ...
        load_dir_btn.setStyleSheet(load_btn.styleSheet())
        load_dir_btn.clicked.connect(self.load_dicom_directory)
        top_layout.addWidget(load_dir_btn)
        
        # Patient info
        self.patient_info_label = QLabel("No DICOM loaded | Select files or directory to begin")
    ...
    def handle_tool_click(self, tool):
        ...
        try:
            ...
            for btn_key, btn in self.tool_buttons.items():
                if btn_key == tool and tool not in ['reset', 'invert', 'crosshair', 'info']:
    ...
    def on_mouse_move(self, x, y):
        ...
        try:
            ...
            if self.active_tool == 'pan':
                ...
                # More responsive windowing
                self.window_width = max(1, self.window_width + dx * 4)
                self.window_level = max(-1000, min(1000, self.window_level + dy * 4))
                self.drag_start = (data_x, data_y)
    ...
    def load_dicom_data(self, file_paths):
        """Load DICOM data with improved error handling"""
        try:
            self.dicom_files = []
            failed_files = []
    ...
    def update_display(self):
        """Improved display update with better error handling"""
        if not self.dicom_files or self.current_image_index >= len(self.dicom_files):
            logger.warning("No DICOM files to display")
            return
        ...
        try:
            ...
            if not hasattr(self.current_dicom, 'pixel_array'):
                logger.error("Current DICOM has no pixel array")
                self.show_error("Current DICOM file has no image data")
                return
            
            # Get pixel data with error handling
            try:
                pixel_data = self.current_dicom.pixel_array
                if pixel_data is None:
                    raise ValueError("Pixel array is None")
                logger.debug(f"Pixel data shape: {pixel_data.shape}, dtype: {pixel_data.dtype}")
            except Exception as e:
                logger.error(f"Error accessing pixel data: {e}")
                self.show_error(f"Error accessing image data: {str(e)}")
                return
            
            # Handle different pixel data formats
            if len(pixel_data.shape) > 2:
                if len(pixel_data.shape) == 3:
                    # Take first slice if 3D, or convert RGB to grayscale
                    if pixel_data.shape[2] == 3:  # RGB
                        pixel_data = np.dot(pixel_data[...,:3], [0.2989, 0.5870, 0.1140])
                    else:
                        pixel_data = pixel_data[:, :, 0]
                else:
                    logger.warning(f"Unexpected pixel data shape: {pixel_data.shape}")
            ...
            if self._cached_image_params == cache_params and self._cached_image_data is not None:
                ...
                logger.debug("Generating new image data")
            ...
            self._image_object = self.canvas.ax.imshow(
                image_data, 
                cmap='gray', 
                origin='upper', 
                extent=(0, w, h, 0),
    ...
    def apply_windowing(self, image_data):
        """Apply window/level to image data with improved handling"""
        try:
            ...
            # Return original data normalized to 0-255
    ...
    def draw_measurements(self):
        ...
        try:
            for measurement in self.measurements:
                ...
                # Convert to physical units if pixel spacing available
                if self.current_dicom and hasattr(self.current_dicom, 'PixelSpacing'):
                    try:
                        ...
                        logger.warning(f"Error calculating physical distance: {e}")
    ...
    def update_overlays(self):
        """Update overlays without full redraw"""
        try:
            # Remove overlay elements only
            lines_to_remove = []
            texts_to_remove = []
            
            for line in self.canvas.ax.lines:
                if line.get_color() in ['red', 'yellow', 'cyan']:
        ...
        try:
            if self.dicom_files:
                self.wl_label.setText(
                    f"WW: {int(self.window_width)}\n"
                    f"WL: {int(self.window_level)}\n"
                    f"Slice: {self.current_image_index + 1}/{len(self.dicom_files)}"
                )
            else:
...
def main():
    ...
    try:
        ...
        logger.info("DICOM Viewer started successfully")
        
        # Start the application event loop
        sys.exit(app.exec_())
    except Exception as e:
        logger.critical(f"Failed to start application: {e}")
        QMessageBox.critical(None, "Startup Error", f"Failed to start DICOM Viewer:\n{str(e)}")
